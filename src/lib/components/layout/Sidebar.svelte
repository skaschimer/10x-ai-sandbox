<script lang="ts">
	...
	const initChatList = async () => {
		...
		if (search) {
			await chats.set(await getChatListBySearchText(search, $currentChatPage));
		} else {
	...
	const loadMoreChats = async () => {
		...
		if (search) {
			newChatList = await getChatListBySearchText(search, $currentChatPage);
		} else {
			newChatList = await getChatList($currentChatPage);
		}
	...
	const searchDebounceHandler = async () => {
		...
		if (search === '') {
			...
			searchDebounceTimeout = setTimeout(async () => {
				allChatsLoaded = false;
				currentChatPage.set(1);
				await chats.set(await getChatListBySearchText(search));
...
<div
	...
	class="h-screen max-h-[100dvh] min-h-screen select-none {$showSidebar
		...
		: '-translate-x-[260px] w-[0px]'} sidebar-container dark:bg-gray-950 dark:text-gray-200 text-sm transition fixed z-50 top-0 left-0 overflow-x-hidden
        "
...
>
	...
	>
		...
		{#if $user?.role === 'admin' || $user?.permissions?.workspace?.models || $user?.permissions?.workspace?.knowledge || $user?.permissions?.workspace?.prompts || $user?.permissions?.workspace?.tools}
			<div class="sidebar-main-label px-1.5 flex justify-center dark:text-gray-200">
				<a
					class="flex-grow flex space-x-3 rounded-lg px-2 py-[7px] hover:bg-steel-200 dark:hover:bg-gray-900 transition"
					href="/workspace"
					on:click={() => {
						selectedChatId = null;
						chatId.set('');
						...
						on:drop={async (e) => {
							...
							if (type === 'chat') {
								...
								if (chat) {
									...
									if (!chat.pinned) {
										const res = await toggleChatPinnedStatusById(chat.id);
									}
						...
						{#if $chats}
							{#each $chats as chat, idx}
								{#if idx === 0 || (idx > 0 && chat.time_range !== $chats[idx - 1].time_range)}
									<div
										class="sidebar-text w-full pl-2.5 text-xs dark:text-gray-500 font-medium {idx ===
										0
											? ''
											: 'pt-5'} pb-1.5"
									>
